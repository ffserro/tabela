...
with col1:
    ...
    st.markdown(f'<h2>{geral_corrida[pd.to_datetime(dt.today())]}</h2>', unsafe_allow_html=True)
    st.divider()    
...
with col2:
    ...
    st.markdown(f'<h2>{geral_corrida[pd.to_datetime(dt.today())].iloc[0]}</h2>', unsafe_allow_html=True)
    st.divider()  
