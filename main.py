...
with col1:
    ...
    st.markdown(f'<h2>{geral_corrida[pd.to_datetime(dt.today())].NOME.iloc[0]}</h2>', unsafe_allow_html=True)
    st.divider()    
...
with col2:
    ...
    st.markdown(f'<h2>{geral_corrida[pd.to_datetime(dt.today() + td(days=1))].NOME.iloc[0]}</h2>', unsafe_allow_html=True)
    st.divider()  
